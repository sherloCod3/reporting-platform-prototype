...
export function useReportEditor(initialData?: {
  title?: string;
  description?: string;
  ...
  const [state, dispatch] = useReducer(editorReducer, {
    ...
    historyIndex: 0,
    // Usa o maior ID existente + 1 para evitar colisoes ao carregar relatorio
    nextId:
  ...
  const loadReport = useCallback(
    (data: { title: string; description: string; components: Component[] }) => {
