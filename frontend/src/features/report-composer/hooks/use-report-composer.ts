/**
 * useReportComposer — Reducer-based state management for section composition.
 *
 * Follows the same reducer pattern as the existing useReportEditor hook.
 * Manages sections as an ordered vertical stack, with history for undo/redo.
 */
...
// ---------------------------------------------------------------------------
// UUID Generator (avoids external dependency)
// ---------------------------------------------------------------------------
...
/** Creates a new section with sensible defaults based on the type */
...
/** Recalculates order values after mutation to keep them sequential */
    ...
    switch (action.type) {
        ...
        case "REMOVE_SECTION": {
            const updated = reindex(
                state.sections.filter((s) => s.id !== action.id)
            );
            const nextSelected =
        ...
        case "REORDER_SECTION": {
            ...
            // Boundary check — can't move beyond edges
...
/** Return type for the useReportComposer hook */
...
/**
 * Manages the composition state of a section-based report.
 * Provides stable callbacks for all mutation operations.
 */
export function useReportComposer(
    ...
    /** Derived: currently selected section object */
