...
export function AuthProvider({ children}: { children: React.ReactNode}) {
    const [token, setToken] = useState<string | null>(() => localStorage.getItem('@qreports:token')); // token em memória (lazy initialization)
    ...
    const value = useMemo(() => ({
        ...
        login,
        logout,
    }), [user, client, token, isAuthenticated]); // memo do contexto
