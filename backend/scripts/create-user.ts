...
async function main() {
    ...
    try {
        ...
        const role = await question('Role (admin/user/viewer) [default: user]: ') || 'user';
        ...
        const [ clients ]: any = await pool.execute('SELECT id, slug FROM clients WHERE active = 1');

        if (clients.length === 0) {
            console.error('No active clients found. Please create a client in the database first.');
            process.exit(1);
        }
