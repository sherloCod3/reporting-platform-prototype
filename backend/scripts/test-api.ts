...
async function main() {
  ...
  try {
    ...
    const email =
      (await question('Email (admin@example.com): ')) || 'admin@example.com';
    ...
    while (true) {
      ...
      const method = (
        (await question('Method (GET/POST/PUT/DELETE) [GET]: ')) || 'GET'
      ).toUpperCase();
      const endpoint =
        (await question('Endpoint (/api/users): ')) || '/api/users';
      ...
      try {
        ...
        if (reqErr.response) {
          console.error(
            `\nErro - Status: ${reqErr.response.status} ${reqErr.response.statusText}`
          );
          console.error('Error Data:', reqErr.response.data);
        } else {
          console.error('\nErro na requisicao:', reqErr.message);
        }
      }
