...
async function verify() {
    ...
    try {
        const [ rows ] = await authPool.execute<any[]>('SELECT db_name, db_port, slug FROM clients WHERE active = 1 LIMIT 1');
        ...
        log({
            step: 'connecting',
            config: { ...clientConn, password: clientConn.password ? clientConn.password.substring(0, 3) + '***' : 'undefined' },
